...
async fn test_status() {
    let app = test::init_service(App::new().route("/status", web::get().to(handlers::status))).await;
...
async fn test_set_name() {
    let app = test::init_service(App::new().route("/a/{name}", web::get().to(handlers::set_name))).await;
